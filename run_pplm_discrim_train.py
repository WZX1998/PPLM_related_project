...
class Discriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        if pretrained_model.startswith("gpt2"):
            ...
            self.encoder = AutoModelForSequenceClassification.from_pretrained(pretrained_model)
            self.embed_size = self.encoder.config.hidden_size
        else:
            raise ValueError(
...
def train_discriminator(
    ...
    if dataset == "mnli":
        ...
        idx2class = ["entailment", "neutral", "contradiction"]
        class2idx = {c: i for i, c in enumerate(idx2class)}
        ...
        dataset = datasets.load_dataset('SetFit/mnli')
        ...
        y = []
        for t in train_data:
            seq = '<|BOS|>'+t['text1']+'<|SEP|>'+t['text2']+'<|endoftext|>'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_model", type=str, default="gpt2-medium",
                        help="Pretrained model to use as encoder") # "roberta-large-mnli"
    ...
    parser.add_argument("--output_fp", default="/content/gdrive/MyDrive/PPLM-master/",
